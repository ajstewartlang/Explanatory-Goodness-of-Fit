...
```{r}
m_con_a_3 <- mixed(Consequent ~ acc_z + (1|P.s) + (Fit|| Item), drp_r, family = binomial, method = "LRT", progress = FALSE, expand_re = TRUE, all_fit = TRUE)
summary(m_con_a_3)
```
...
```{r}
m_con_a_3_pb <- mixed(Consequent ~ acc_z + (1|P.s) + (Fit|| Item), drp_r, family = binomial, method = "PB", progress = FALSE, expand_re = TRUE, all_fit = TRUE)
nice(m_con_a_3_pb)
```

If we plot the estimated effect we can see that it indeed is small.

```{r, fig.width=4, fig.height=3}
plot(Effect("acc_z", m_con_a_3_pb$full_model, 
...
summary(m_spill_a_2)
```
Aa one random slopes is estimated to be zero, we consequently refit the model without this parameter.

```{r}
m_spill_a_3 <- mixed(spillover ~ acc_z + (1|P.s) + (Fit|| Item), drp_r, family = binomial, method = "LRT", progress = FALSE, expand_re = TRUE)
summary(m_spill_a_3)
```
...
The effect of `acc_z` is not significant in either model.
